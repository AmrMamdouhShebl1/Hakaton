...
async def get_relevant_properties(query: str, top_k: int = 5):
    ...
    Always use the English vector store regardless of query language.
    """
    if not vector_store_english:
        logger.warning("English vector store not available")
        return []

    # Get the most relevant documents
    try:
        docs = vector_store_english.similarity_search(query, k=top_k)
        return docs
    except Exception as e:
...
- Key features
- Brief description
Present URLs as proper clickable links using markdown format: [Click here]
...
- الميزات الرئيسية
- وصف موجز
قدم الروابط باستخدام تنسيق ماركداون الصحيح: [اضغط هنا]
...
async def generate_response(user_message: str, chat_history: List[Dict[str, str]]) -> str:
    ...
    # Get relevant properties - always from English database
